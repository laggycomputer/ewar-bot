    ...
    pub(crate) async fn short_summary(&self, mongo: &Database) -> Result<Box<str>, BotError> {
        let summary = match &self.inner {
            ...
            StandingEventInner::JoinLeague {
                ...
                for player_id in victims {
                    looked_up.push(
                        try_lookup_player(mongo, SystemID(*player_id))
                            .await?
                            .expect("user joined to league not found"),
                    );
                }
            ...
            StandingEventInner::Penalty {
                ...
                for player_id in victims {
                    looked_up.push(
                        try_lookup_player(mongo, SystemID(*player_id))
                            .await?
                            .expect("penalized user not found"),
                    );
                }

                format!(
                    "{} penalized {} rating for {reason}",
                    looked_up.into_iter().map(|u| u.short_summary()).join(", "),
                    -delta_rating
                )
            }
            StandingEventInner::InactivityDecay {
                ...
                format!(
                    "{} gain {} deviation due to 1 week of inactivity",
                    looked_up.into_iter().map(|u| u.short_summary()).join(", "),
                    delta_deviation
                )
            }
        ...
        Ok(
            (if self.approval_status.as_ref().is_some_and(|st| !st.approved) {
                format!("~~{summary}~~")
            } else {
                summary
            })
