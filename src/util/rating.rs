...
impl RatingExtra for TrueSkillRating {
    ...
    fn format_rating(&self) -> String {
        format!(
            "{:.2}{}",
            self.leaderboard_rating(),
            if self.is_provisional() { "**?**" } else { "" }
        )
    }
...
pub(crate) async fn advance_approve_pointer(
    ...
    while let Some(standing_event) = allegedly_unreviewed.next().await {
        if first_unreviewed_event_number_num >= stop_before.unwrap_or(EventNumber::MAX) {
            break;
        }
...
impl StandingEvent {
    pub(crate) async fn process_effect(&self, mongo: &Database) -> Result<(), BotError> {
        let inner_processable = match self.inner {
            Penalty { .. } => &self.inner.clone().try_into_generic_variant().expect("1984"),
            _ => &self.inner,
        };
