...
#[poise::command(
    prefix_command,
    slash_command,
    subcommands("post", "whatif", "query", "log")
)]
...
pub(crate) async fn post(
    ...
    let poster_info = try_lookup_player(&ctx.data().mongo, DiscordID(ctx.author().id.get()))
        .await?
    ...
    let initial_confirm_button = CreateButton::new("postgame_confirm_initial")
    ...
    let waited = msg_part2
        .message()
        .await?
        .await_component_interaction(&ctx.serenity_context().shard)
        .author_id(ctx.author().id)
        .custom_ids(vec![String::from("postgame_confirm_initial")])
        .timeout(Duration::from_secs(initial_confirm_timeout))
        .await;

    if waited.is_none() {
        ctx.reply("timed out, submission of this game cancelled")
            .await?;
        return Ok(());
    }

    let mut not_signed_off = placement_discord
        .clone()
        .into_iter()
        .collect::<HashSet<_>>();
        ...
        .create_response(
            ...
            CreateInteractionResponse::UpdateMessage(
                CreateInteractionResponseMessage::new()
                    .embed(base_embed(ctx).description(emb_desc))
                    .components(vec![]),
            ),
        )
        .await?;
    ...
    if poster_not_moderator {
        let make_signoff_msg = |not_signed_off: &HashSet<User>, disable_button: bool| {
            ...
            **after 5 minutes of inactivity, game is rejected for submission**",
                    ...
                    placement_discord
                        .iter()
                        .map(|user| {
                            if not_signed_off.contains(user) {
        ...
        let mut party_sign_stage_msg = ctx
            .send(
                CreateReply::default()
        ...
        while not_signed_off.len() > max_not_signing {
            ...
            match party_sign_stage_msg
                .await_component_interaction(&ctx.serenity_context().shard)
                .filter(move |ixn| not_signed_off_freeze.contains(&ixn.user))
                ...
                .await
            {
                None => {
                    ...
                    party_sign_stage_msg
                        .edit(
                            ctx.http(),
                            EditMessage::new().components(signoff_components),
                        )
                        .await?;

                    party_sign_stage_msg
                        .reply(ctx.http(), "timed out, this game is voided for submission")
                        .await?;
                ...
                Some(ixn) => {
                    ...
                    let (signoff_content, signoff_components) =
                        make_signoff_msg(&not_signed_off, false);
                    party_sign_stage_msg
                        .edit(
                            ctx.http(),
                            EditMessage::new()
...
pub(crate) async fn query(
    ...
    let StandingEvent {
        ...
        return Err("game-looking struct is not a game".into());
    };
            ...
            .embed(
                base_embed(ctx)
                    ...
                    .field(
                        ...
                        format!(
                            "{:02}:{:02}",
                            chrono_game_length.num_minutes(),
                            chrono_game_length.num_seconds() % 60
                        ),
                        true,
                    )
                    .description(ranking),
