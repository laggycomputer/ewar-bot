...
pub(crate) async fn try_lookup_player(
    mongo: &Database,
    how: UserLookupType<'_>,
) -> Result<Option<Player>, BotError> {
    Ok(mongo
...
async fn display_lookup_result(ctx: Context<'_>, looked_up: Player) -> Result<(), BotError> {
    ...
    let win_loss = ctx
        ...
        .with_type::<WinLossAggregate>()
        .await?
        .try_next()
        .await?
        .unwrap_or(WinLossAggregate { wins: 0, losses: 0 });
    ...
    time_formatter.num_items(2).min_unit(Minutes);
...
#[poise::command(
    prefix_command,
    slash_command,
    subcommands("by_discord", "by_username", "by_id")
)]
...
async fn by_discord(
    ctx: Context<'_>,
    #[description = "Discord user to lookup by"] user: Option<User>,
) -> Result<(), BotError> {
    let user = user.as_ref().unwrap_or(ctx.author());

    match try_lookup_player(&ctx.data().mongo, UserLookupType::DiscordID(user.id.into())).await? {
        None => {
            ctx.reply("could not find player with that discord user")
                .await?;
        }
        Some(looked_up) => display_lookup_result(ctx, looked_up).await?,
    }
...
async fn by_username(
    ctx: Context<'_>,
    #[description = "System handle to lookup by"] handle: String,
) -> Result<(), BotError> {
...
async fn by_id(
    ctx: Context<'_>,
    #[description = "System ID to lookup by"] id: PlayerID,
) -> Result<(), BotError> {
...
pub(crate) async fn register(
    ctx: Context<'_>,
    #[description = "Defaults to your Discord username - name you want upon registration"]
    desired_name: Option<String>,
) -> Result<(), BotError> {
    let proposed_name = desired_name
    ...
    if try_lookup_player(&ctx.data().mongo, Username(&proposed_name))
        ...
        .is_some()
    {
        ctx.reply(format!("user by name {proposed_name} already exists"))
            .await?;
        return Ok(());
    }
